...
get.excel.to.rda.if.required <- function(url.addr, data.path, table.name, file.format) {
    # Fetch excel spreadsheet and save all sheets listed in the first sheet of a
    # given file
    ...
    if (!file.exists(dest.path, showWarnings = F)[1]) {
        ...
        if (missing(table.name)) {
            # Reading all the sheets specified in the first sheet, only makes sense for this
            # exersise
