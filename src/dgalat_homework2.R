...
plot.q1 <- function() {
    ...
    pl + geom_bar(stat = "identity") + xlab("Party") + ylab("Proportion of voters") + ggtitle("Voters per party in Alaska")
    
    pl <- ggplot(AK, aes(x = AK$`LAST NAME`, y = AK$`GENERAL %`))
    pl + geom_bar(stat = "identity") + xlab("Party") + ylab("Proportion of voters") + ggtitle("Voters per candidate in Alaska")
    
    aggr.dat <- aggregate(IN$`GENERAL %` ~ IN$PARTY, FUN = sum)
    pl <- ggplot(aggr.dat, aes(x = aggr.dat[ ,1], y = aggr.dat[ ,2]))
    pl + geom_bar(stat = "identity") + xlab("Party") + ylab("Proportion of voters") + ggtitle("Voters per party in Indiana")
...
plot.q2 <- function() {
    ...
    # scatter party
    ggplot(q2.aggr, aes(x = party, y = votes, color = party)) +
        geom_point(shape = 1, alpha = 3/4) +
        theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
    ...
    plot(as.factor(find.party.name.from.abbr(q2.aggr$party)), 
         q2.aggr$votes, xlab= "Parties", ylab = "Number of voters in a state", cex.axis = 0.45, las = 2)
    ...
    plot(as.factor(q2$`LAST NAME`), 
         q2$`GENERAL RESULTS`, xlab= "Parties", ylab = "Number of voters in a state", cex.axis = 0.45, las = 2)  
}
...
# Question 6
# I want to check if Democrates will lose, so we test if they will recieve 50% of votes

#test the claim that Democrates will lose. 
#Does the evidence from sample support the claim?

#Ho: p0=0.05 Democrats will win election, given that they receive only 50% of votes
#vs Ha: p0 is not 0.05,  

#let x- proportion # of wins of D over the sample of n periods, with hypothesised p0=0.05 prob of democrats winning
...
# this is a test for comparing independent samples (!)
# we only have one sample, democrats and republican votes are not independent
# therefore we use binomial proportion test
...
# Type 2 error, rejecting Ha, given that Ha is true
# same as the probability that observed value is not the rejection region when Ha is true
...
#The large value of β  tells us that sample of size n = 51 frequently will fail to detect a difference of 0.1 from the hypothesized proportion p=0.5
#which means that HT (based on the given input) has a weak power and shows suboptimal results by passing Ho hypothesis; one of the solutions is to increase sample size n
...
# } else {
#     print("There is enough evidence to reject Ho, because sample mean significantly deviates from mu=0.5")
# }
...
# Goodness of fit, we are testing that votes are equally distributed between parties
# this is an equivalent of saying mu = 0.5
...
plot.q10 <- function() {
    ...
    abline(lm(as.numeric(df.q10$value[df.q10$party == "O"]) ~ as.numeric(df.q10$year[df.q10$party == "O"])), col = "green")
}